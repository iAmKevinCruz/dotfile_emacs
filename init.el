...
(use-package emacs
  ...
  (fill-column 100)                               ;; The column line to know when to wrap
  (org-export-with-sub-superscripts '{})          ;; To use sub and superscript with _{} and ^{}
  (org-use-sub-superscripts '{})                  ;; To use sub and superscript with _{} and ^{}
...
;;; DIRVISH
;; Dirvish is a modern file manager that extends and enhances Dired with
;; features like file previews, icons, and a more intuitive interface.
;; It provides a more visual and feature-rich experience compared to vanilla Dired,
;; similar to ranger or lf file managers.
(use-package dirvish
  :ensure t
  :straight t
  :init
  ...
  (dirvish-quick-access-entries ;; Set quick access directories
   '(("h" "~/"                "Home")
  ...
  :config
  ;; Enable mouse support in Dirvish
...
(use-package org
  ...
  (setq org-todo-keywords
        '((sequence "TODO(t)" "WIP(p)" "AWAP(a)" "|" "DONE(d)" "CANCELED(c)" "HOLD(h)")
          (sequence "REPORT(r)" "BUG(b)" "BUGWIP(w)" "KNOWNCAUSE(k)" "|" "FIXED(f)")))
...
;; DISABLED: Not needed for org-mode only usage - saves ~36% file opening time
...
;; CAPE
;; this is the completion menu at point
(use-package cape
...
(use-package org-roam
  ...
  ;; Configure org-id to use timestamp format matching Neovim setup
  (setq org-id-method 'ts)  ; Use timestamp for IDs
  (setq org-id-ts-format "%Y%m%d%H%M%S")  ; Format: 20251114214329
  :custom
  (org-roam-directory "~/org")
  (org-roam-completion-everywhere t)
  ;; Configure capture templates to match Neovim setup
  (org-roam-capture-templates
    '(("d" "default" plain
       "%?"
       :target (file+head "0 Inbox/${title}.org"
                          ":PROPERTIES:\n:ID: %(org-id-new)\n:END:\n#+title: ${title}\n\n")
       :unnarrowed t)
      ("p" "people" plain
       "%?"
       :target (file+head "Extras/People/${title}.org"
                          ":PROPERTIES:\n:ID: %(org-id-new)\n:END:\n#+title: ${title}\n\n")
       :unnarrowed t)
      ("P" "project" plain
       "%?"
       :target (file+head "1 Projects/${title}.org"
                          ":PROPERTIES:\n:ID: %(org-id-new)\n:END:\n#+title: ${title}\n\n")
       :unnarrowed t)
      ("m" "meeting" plain
       "%?"
       :target (file+head "Timestamps/Meetings/%<%Y-%m-%d> ${title}.org"
                          ":PROPERTIES:\n:ID: %(org-id-new)\n:END:\n#+title: %<%Y-%m-%d> ${title}\n#+filetags: :meeting:\n\n")
       :unnarrowed t)))
  ;; Configure dailies to match Neovim setup
  (org-roam-dailies-directory "Timestamps/Journal")
  (org-roam-dailies-capture-templates
    '(("d" "default" entry
       "%?"
       :target (file+head "%<%Y-%m-%d-%a>.org"
                          ":PROPERTIES:\n:ID: %(org-id-new)\n:END:\n#+title: %<%Y-%m-%d %A>\n\n"))))
  ; :bind (("SPC n l" . org-roam-buffer-toggle)
...
(use-package evil
  ...
  (evil-define-key 'normal 'global (kbd "<leader> f w") 'consult-grep)
  (evil-define-key 'normal 'global (kbd "<leader> f g") 'consult-git-grep)
  (evil-define-key 'normal 'global (kbd "<leader> f r") 'consult-ripgrep)
  (evil-define-key 'normal 'global (kbd "<leader> f h") 'consult-info)
  (evil-define-key 'normal 'global (kbd "<leader> /") 'consult-line)

  ;; Orgmode keybindings
  (evil-define-key 'normal 'global (kbd "<leader> o t h") 'org-toggle-heading)
  (evil-define-key 'normal 'global (kbd "<leader> o i h") 'org-insert-heading-after-current)
  (evil-define-key 'normal 'global (kbd "<leader> o i H") 'org-insert-subheading)
  (evil-define-key 'normal 'global (kbd "<leader> o i t") 'org-insert-todo-heading)
  ...
  ;; Avy keybindings for Evil mode
  (evil-define-key 'normal 'global (kbd "s") 'avy-goto-char-timer)  ;; Quick jump with 's' in normal mode
  (evil-define-key 'visual 'global (kbd "s") 'avy-goto-char-timer)  ;; Quick jump with 's' in visual mode
  (evil-define-key 'operator 'global (kbd "s") 'avy-goto-char-timer) ;; Quick jump with 's' in operator mode
...
;;; AVY
;; The `avy' package provides a quick way to jump to visible text using
;; character-based hints. It's similar to vim-easymotion, ace-jump-mode,
;; or vim-sneak, allowing you to quickly navigate to any visible part
;; of the buffer by typing one or two characters.
(use-package avy
  :ensure t
  :straight t
  :defer t
  :config
  ;; Smart function to use avy-isearch when in isearch mode,
  ;; otherwise use avy-goto-char-timer
  (defun ek/avy-smart-goto ()
    "Use avy-isearch if in isearch mode, otherwise use avy-goto-char-timer."
    (interactive)
    (if isearch-mode
        (avy-isearch)
      (avy-goto-char-timer)))

  ;; Configure avy settings
  (setq avy-timeout-seconds 0.3)  ;; Timeout for avy-goto-char-timer
  (setq avy-all-windows t)         ;; Jump across all visible windows
  ; (setq avy-background t)          ;; Dim background when avy is active

  :bind
  ;; Global binding for M-s s
  (("M-s s" . ek/avy-smart-goto)
   ;; Additional useful avy bindings (optional)
   ("M-s c" . avy-goto-char-2)    ;; Jump to 2 chars
   ("M-s l" . avy-goto-line)      ;; Jump to line
   ("M-s r" . avy-resume)         ;; Resume last avy
   ("M-s w" . avy-goto-word-1))   ;; Jump to word

  :bind
  ;; Binding within isearch-mode-map for when already in isearch
...
;;; CATPPUCCIN THEME (COMMENTED OUT - USING DOOM-THEMES INSTEAD)
...
;;; DOOM-THEMES
;; The `doom-themes' package provides a collection of beautiful themes
;; inspired by Doom Emacs. These themes are designed to be modern,
;; visually appealing, and optimized for readability across various
;; programming languages and Emacs modes.
(use-package doom-themes
  ...
  ;; Load the doom-badger theme
  (load-theme 'doom-badger t)

  ;; Enable flashing mode-line on errors
  (doom-themes-visual-bell-config)

  ;; Enable custom org-mode font settings
  (doom-themes-org-config)

  ;; After loading the theme, reapply custom faces to ensure they override theme settings
  ;; This ensures your org-verbatim customization persists
...
;;; https://sophiebos.io/posts/beautifying-emacs-org-mode/#prettier-ui-elements
;; Configure fonts and visual settings for org-mode to make it more visually appealing
(defun ek/org-mode-visual-setup ()
  "Set up visual enhancements for org-mode."
  ...
  ;; Configure org-indent faces
  (require 'org-indent)
  (set-face-attribute 'org-indent nil :inherit '(org-hide fixed-pitch))

  ;; Set fixed-pitch for code blocks and related elements
  ...
  ;; Enable variable-pitch-mode for better readability
...
;; ;; Add the visual setup to org-mode-hook
;; (add-hook 'org-mode-hook #'ek/org-mode-visual-setup)
;;
...
;;; ORG-SUPERSTAR
;; Org-superstar provides fancy bullets and other visual enhancements for org-mode
(use-package org-superstar
  :ensure t       ; Ensure the package is installed
  :straight t     ; Use straight.el to manage this package
  :defer t        ; Load only when needed
  :hook (org-mode . org-superstar-mode)  ; Enable when entering org-mode
  :config
...
; Have Emacs work with my shell so programs like fd work in emacs no matther where emacs is opened from
; (when (memq window-system '(mac ns))
;   (setq shell-file-name "/bin/zsh") ;; or "/bin/bash" depending on your default shell
;   (exec-path-from-shell-initialize))
(add-to-list 'exec-path "/opt/homebrew/bin/")
(setenv "PATH" (concat (getenv "PATH") ":/opt/homebrew/bin"))

;;; Preload org-mode for faster first-file opening
;; Since we only use Emacs for org-mode, preload it after startup
;; This eliminates the delay when opening the first org file
